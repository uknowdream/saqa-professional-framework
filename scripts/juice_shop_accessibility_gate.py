...
def _assert_loopback_http(url: str) -> None:
    ...
    if (
        ...
        raise ValueError(
            "accessibility target must be credential-free HTTP on an approved loopback host with a port"
        )
            ...
            try:
                ...
                metrics = page.evaluate(
                    """() => {
                      ...
                      const explicitLabel = e => e.id
                        ? [...document.querySelectorAll(`label[for=\"${CSS.escape(e.id)}\"]`)].map(label => text(label)).join(' ')
                        : '';
                      ...
                      const controls = [...document.querySelectorAll(
                        'button,a[href],input,select,textarea,[role=\"button\"],[role=\"link\"],[role=\"checkbox\"],[role=\"radio\"],[role=\"switch\"],[role=\"combobox\"],[role=\"textbox\"]'
                      )].filter(rendered).filter(e => !(e.tagName.toLowerCase() === 'input'
                        && (e.getAttribute('type') || '').toLowerCase() === 'hidden'));
                      const unnamed = controls.filter(e => !name(e)).map(e => ({
                        tag: e.tagName.toLowerCase(), id: e.id || '', role: e.getAttribute('role') || '',
                        type: e.getAttribute('type') || '', tab_index: e.tabIndex, outerHTML: e.outerHTML.slice(0, 300)
                      }));
                      return {
                        lang_present: !!(document.documentElement.getAttribute('lang') || '').trim(),
                        title_present: !!document.title.trim(),
                        images_missing_alt: [...document.images].filter(rendered).filter(e => !e.hasAttribute('alt')).length,
                        interactive_control_count: controls.length,
                        unnamed_interactive_controls: unnamed.length,
                        unnamed_control_details: unnamed
                      };
                    }"""
                )
                evidence["details"] = {
                ...
                if not response or response.status < 200 or response.status >= 400:
                    raise AssertionError(
                        f"expected successful page response, got {response.status if response else None}"
                    )
                ...
                evidence["details"]["independent_oracle"] = {
                    "engine": "axe-core",
                    "rules": AXE_RULES,
                    "violation_count": len(oracle),
                    "violations": oracle,
                }
                disposition = _classify_heuristic_finding(
                    metrics["unnamed_control_details"], len(oracle)
                )
                ...
                if metrics["images_missing_alt"]:
                    failures.append(f"{metrics['images_missing_alt']} rendered image(s) lack alt")
                if oracle:
                    failures.append(f"axe-core found {len(oracle)} selected rule violation(s)")
                if disposition == "INCONCLUSIVE":
                    failures.append(
                        f"{metrics['unnamed_interactive_controls']} user-operable DOM-heuristic unnamed control(s) require independent oracle correlation"
                    )
                if failures:
                    raise AssertionError("; ".join(failures))

                # Cleanup is inside the Playwright context so PASS cannot leak browser processes.
                context.close()
                context = None
                browser.close()
                browser = None
