...
def main()->int:
 ...
 try:
  ...
  with sync_playwright() as playwright:
   ...
   evidence["details"]["independent_oracle"]={"engine":"axe-core","rules":AXE_RULES,"violation_count":len(oracle),"violations":oracle}; disposition=_classify_heuristic_finding(metrics["unnamed_control_details"],len(oracle)); evidence["details"]["heuristic_disposition"]=disposition
   failures=[]
   ...
   evidence["status"]="PASS"; context.close()
 ...
 print(json.dumps(evidence,indent=2,sort_keys=True)); return 0 if evidence["status"]=="PASS" else 1
