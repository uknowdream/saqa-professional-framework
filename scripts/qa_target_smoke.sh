...
payload = {
    ...
    "status": "FAIL",
    "exit_code": rc,
    "details": "target smoke failed before completion; inspect CI diagnostics",
}
...
wait_http() {
  local name="$1" url="$2" max_attempts="${3:-60}" attempt=1 delay=2 status=""
...
result = {"schema": "saqa.target-smoke.v3", "timestamp_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "compose_file": compose, "targets": ["owasp-juice-shop", "owasp-webgoat"], "http_methods": ["GET"], "redirects_followed": False, "destructive_actions": False, "runner": platform.platform(), "status": "PASS"}
